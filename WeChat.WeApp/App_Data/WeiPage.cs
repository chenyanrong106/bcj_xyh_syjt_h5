...
public class WeiPage : System.Web.UI.Page
{
    ...
    public string Token(string ToUserName)
    {
        ...
        if (m.Access_token != "")
        {
            ...
            string url = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=" + Access_token + "&openid=" + (m.OneOpenID == null ? "oS7pm1hhcQ3grdAaRe8NEXZJn7jY" : m.OneOpenID);
    ...
    public OAauth_Log SaveOA(string FromUserName, string ToUserName)
    {
        ...
        if (oa == null)
        {
            ...
            mss.SaveOA(oa);
            if (!string.IsNullOrEmpty(oa.headimgurl))
            {
                DownHeadImage(oa);
            }
    ...
    public static ORG_INFO CurrentMerchants
    {
        get
        {
            ...
            {
                return (ORG_INFO)(HttpContext.Current.Session["CurrentMerchants"]);
            }
            else
            {
                ...
                {
                    var cuo = CLoginService.GetMerchants(Convert.ToInt32(CurrentLoginUser.ID));
                    if (cuo != null)
                    {
    ...
    public static USER_INFO_EX CurrentLoginUser
    {
        get
        {
            ...
            {
                return (USER_INFO_EX)(HttpContext.Current.Session["LoginUseInfo"]);
            }
            else
            {
                return null;
    ...
    public string CurrentToUserName
    {
        get
        {
            if (!string.IsNullOrEmpty(System.Web.HttpContext.Current.Request.QueryString["ToUserName"]))
            {
                System.Web.HttpContext.Current.Session["ToUserName"] = System.Web.HttpContext.Current.Request.QueryString["ToUserName"].ToString();
                return System.Web.HttpContext.Current.Request.QueryString["ToUserName"];
            }
            if (System.Web.HttpContext.Current.Session["ToUserName"] != null)
                return System.Web.HttpContext.Current.Session["ToUserName"].ToString();
            return mjuserid;
        }
    }

    /// <summary>
    /// 获取微店信息 供前台页面调用 优先从System.Web.HttpContext.Current.Session中获取
    /// </summary>
    public ORG_INFO WD
    {
        get
        {
            if (System.Web.HttpContext.Current.Session["WD"] != null)
            {
                return (ORG_INFO)System.Web.HttpContext.Current.Session["WD"];
            }
            else
            {
                if (CurrentToUserName != "")
                {
                    ORG_INFO WD = CLoginService.GetMerchants(CurrentToUserName);
        ...
        if ((System.Web.HttpContext.Current.Session["ToUserName"] == null && System.Web.HttpContext.Current.Session["FromUserName"] == null) || (System.Web.HttpContext.Current.Session["FromUserName"] != null && mss.GetISOA(System.Web.HttpContext.Current.Session["FromUserName"].ToString()) == 0))
        {
            ...
            if (System.Web.HttpContext.Current.Request.QueryString["code"] != null)
            {
                try
                {
                    ...
                    url = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + GetAppid(tousername) + "&secret=" + GetSecret(tousername) + "&code=" + (System.Web.HttpContext.Current.Request.QueryString["code"].ToString().Split(',').Length > 1 ? System.Web.HttpContext.Current.Request.QueryString["code"].ToString().Split(',')[1] : System.Web.HttpContext.Current.Request.QueryString["code"].ToString().Split(',')[0]) + "&grant_type=authorization_code";
                    string token = PostRequest(url);
                    //  mss.SaveLog(new WXLOG { CON = token, TIME = DateTime.Now });
                    if (token.Contains("7200") || token.Contains("expires_in"))
                    {
    ...
    /// </summary>
    /// <returns></returns>
    public string mjuserid { get { return "gh_3161af11269f"; } }
