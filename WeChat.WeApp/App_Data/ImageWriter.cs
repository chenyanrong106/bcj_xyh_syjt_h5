...
public class ImageWriter
{
    ...
    {
        return SetTransparence(new Bitmap(System.Drawing.Image.FromStream(new System.IO.MemoryStream(System.IO.File.ReadAllBytes(readpath)))), transparence);
    }
    #endregion 设置透明度结束
    #region 添加水印开始
    /// <summary>
    ...
    /// <param name="margin">水印边距: int</param>
    /// <param name="writepath">保存路径(包含完整路径,文件名及其扩展名): string</param>
    public void SaveWatermark(string readpath, string watermarkpath, string watermarkpath2, string watermarkpath3, float transparence, WatermarkPosition position, int margin, string writepath)
    {
        //if (string.Compare(readpath, writepath) == 0)
        //    throw new ArgumentException("源图片与目标图片地址相同");
        ...
        {
            Bitmap b = new Bitmap(System.Drawing.Image.FromStream(new System.IO.MemoryStream(System.IO.File.ReadAllBytes(readpath))));
            if (b.Width > 500 && b.Height > 720)
            {
                transparence = 0.1f;
            }
            else if (b.Width > 250 && b.Height > 360)
            {
                watermarkpath = watermarkpath3;
            }
            else if (b.Width > 150 && b.Height > 220)
            {
                watermarkpath = watermarkpath2;
            }
            SaveWatermark(new Bitmap(System.Drawing.Image.FromStream(new System.IO.MemoryStream(System.IO.File.ReadAllBytes(readpath)))), SetTransparence(watermarkpath, transparence), position, margin, writepath);
