;(function(window,undefined,$){
	...
	function __BuildRowHtml__(data, options)
	{
	    ...
	    for (var i = 0, l = data.length; i < l && i < options.rp; i++) {
	        ...
	        for (var j = 0 ; j < k ; j++) {
	            ...
	            var title = "";
	            if (col.title) {
	                title = " title='" + data[i].cell[j] + "' ";
	            }
	            html.push("<td style='width:", col.width ? col.width : "auto", "'><div", style.join(""), " " + title + ">", text, "</div></td>");
	        }
	        html.push("</tr>")
	    };	   
	...
	function __BuildPageNavition__(options){
		...
		var html = [];
		html.push("<ul>");
		html.push("<li><span>",options.total,"条",options.page," / ",options.pages," 页</span></li>");
		if (options.page > 1) {
