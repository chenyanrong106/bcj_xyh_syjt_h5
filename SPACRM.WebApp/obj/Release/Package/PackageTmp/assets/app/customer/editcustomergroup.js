; (function (window, undefined, $) {
    $(document).ready(function () {
        ...
        util.Condition1change= function(obj)
        {
            var code2 = obj.value.split("@")[1];
            if (code2 == "1") {
                $("#c3").html("消费次数"); //titlesHtml.push("<label id=\"c3\" class=\"control-label\" style=\"text-align: left\">消费次数</label>");
            } else if (code2 == "11") {
                $("#c3").html("消费金额"); //titlesHtml.push("<label id=\"c3\" class=\"control-label\" style=\"text-align: left\">消费金额</label>");
            }
        }
            ...
            $.ajax({
                ...
                success: function (result) {
                    ...
                    if (type == 1) {
                        ...
                        SetChildrenCondition(1, "PC.NAME", 1);//初始化第一个选项条件
                    ...
                    {
                        var con1 = "<select class=\"form-control\" id=\"Condition1\" name=\"Condition1\" onchange=\"util.Condition1change(this)\">";
                        var con2 = "<select class=\"form-control\" id=\"Condition2\" name=\"Condition2\">";

                        var dts = result.data;

                        for (var i = 0; i < dts.length; i++) {
                            if (dts[i].Code == "1" || dts[i].Code == "11") {
                                con1 += "<option value=\"" + dts[i].Value + "@" + dts[i].Code + "\">" + dts[i].Name + "</option>";
                            }
        ...
        $('#btnAddOneCon').click(function () {
            ...
            var code2 = "";
            if (value1 == 2) {
                value2 = $("#Condition1").val().split("@")[0];//筛选条件下拉列表选中值 --条件1
                name2 = $("#Condition1").find("option:selected").text();//筛选条件下拉列表选中名称  --条件1
                code2 = $("#Condition1").val().split("@")[1];
            } else {
                value2 = $("#Condition1").val();//筛选条件下拉列表选中值 --条件1
                name2 = $("#Condition1").find("option:selected").text();//筛选条件下拉列表选中名称  --条件1
            }
            var valueR = $("#Condition1").val();//筛选条件下拉列表选中值 --条件1
            ...
            var cuName = "空括号_" + name1 + "_" + name2 + "_" + name3 + "_空括号_and_" + nv; //var cuName = "空括号_" + name1 + "_" + name2 + "_" + name3 + "_空括号_and_" + nv;
            var cuValue = "空括号_" + value1 + "_" + valueR + "_" + value3 + "_空括号_and_" + nv;// "空括号_" + value1 + "_" + value2 + "_" + value3 + "_空括号_and_" + nv;
            ...
            else if (value1 == 2)//消费状况
            {
                ...
                if (Type == 1)//动态分组
                {
                    ...
                    } else if (code2 == "11")//消费金额
                    {
                        strSql = " A.ID in(select CUST_ID from ORDER_HEAD where TRANS_DATE>=Convert(nvarchar(10),DateAdd(dd,-'" + parseInt(value2) + "',getdate()),120) and TRANS_DATE<=Convert(nvarchar(10),getdate(),120) group by CUST_ID having SUM(PAYED_AMT) " + value3 + " '" + nv + "') ";
                    }
                }
                else//静态分组
                {
                    ...
                    {
                        strSql = " A.ID in(select CUST_ID from ORDER_HEAD where TRANS_DATE>='" + DateFrom + "' and TRANS_DATE<='" + DateTo + "' group by CUST_ID having SUM(PAYED_AMT) " + value3 + " '" + nv + "') ";
                    }
                }
        ...
        function showConditionList() {
            ...
            if (nameStrs != "") {
                ...
                for (var i = 0; i < names.length; i++) {
                    ...
                    namesHtml.push("<td style=\"width: 15%;\">按 <span class=\"label label-success\">", cuObj[2], "</span> 筛选</td>");

                    namesHtml.push("<td style=\"width: 10%;\">");
                    namesHtml.push("<select  style=\" height:28px;width:60px;\" id=\"startStr" + i + "\" name=\"startStr" + i + "\" onchange=\"util.selectChangeM('" + names[i] + "',1,this);\">");
                    // class=\"form-control\"
                    if (cuObj[1] == "空括号" || cuObj[1] == "undefined") {
                    ...
                    namesHtml.push("<td style=\"width: 15%;\"><div style=\"text-align:left;\">", cuObj[3], "</div></td>");
                    namesHtml.push("<td style=\"width: 10%;\"><div style=\"text-align:left;\">", cuObj[4], "</div></td>");
                    namesHtml.push("<td style=\"width: 20%;\"><div style=\"text-align:left;\">", cuObj[7], "</div></td>");

                    namesHtml.push("<td style=\"width: 10%;\">");
                    namesHtml.push("<select style=\" height:28px;width:60px;\" id=\"endStr" + i + "\" name=\"endStrendStr" + i + "\" onchange=\"util.selectChangeM('" + names[i] + "',2,this);\">");
                    // class=\"form-control\"
                    if (cuObj[5] == "空括号" || cuObj[1] == "undefined") {
                        namesHtml.push("<option value=\"空括号\" selected=\"true\"></option>");
                    }
                    else {
                        namesHtml.push("<option value=\"空括号\"></option>");
                    ...
                    if (result.status == 1) {
                        ...
                        $(json).each(function (i) {
                            ...
                            for (var j = 0; j < json.length; j++) {
                                ...
                                var v1 = json[j].ATTRIBUTE;
                                var value2R = "";
                                if (value1 == 2) {
                                    value2 = v1.split('@')[0];//筛选条件下拉列表选中值 --条件1
                                    value2R = v1.split('@')[1];
                                } else {
                                    value2 = json[j].ATTRIBUTE;//筛选条件下拉列表选中值 --条件1
                                    value2R = "1";
                                ...
                                for (var i = 0; i < ListCondition.length; i++) {
                                    if ((ListCondition[i].Code == value2R) && ListCondition[i].Value == value2) {
                                ...
                                var cuName = json[j].BRACKET_START + "_" + name1 + "_" + name2 + "_" + name3 + "_" + json[j].BRACKET_END + "_" + json[j].CONDITION_R + "_" + nv; //var cuName = "空括号_" + name1 + "_" + name2 + "_" + name3 + "_空括号_and_" + nv;
                                var cuValue = json[j].BRACKET_START + "_" + value1 + "_" + value2 + "_" + value3 + "_" + json[j].BRACKET_END + "_" + json[j].CONDITION_R + "_" + nv;// "空括号_" + value1 + "_" + value2 + "_" + value3 + "_空括号_and_" + nv;
        ...
        $('#btnCancelGC').click(function () {
            window.location.href = "/Customer/CustomerGroup.do";
        });
    });
