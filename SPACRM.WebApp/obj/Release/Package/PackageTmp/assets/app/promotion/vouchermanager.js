    ...
    $(document).ready(function () {
        ...
        var gridopt = {
            ...
            colModel: [
                    ...
                    { display: '券号', name: 'VOUCHER_NO', width: "5%", sortable: false, align: 'left' },
                    { display: '面值', name: 'FACE_VALUE', width: "9%", sortable: false, align: 'left', process: formatRMB },
                    { display: '是否可转让', name: 'IS_TRANSFER', width: "9%", sortable: false, align: 'left', process: formatTRANSFER },
                    { display: '是否可销售', name: 'CAN_SALE', width: "9%", sortable: false, align: 'left', process: formatTRANSFER },
                    { display: '销售价格', name: 'SALE_PRICE', width: "8%", sortable: false, align: 'left' },
                    { display: '发放门店', name: 'STORE_NAME', width: "8%", sortable: false, align: 'left' },
                    ...
                    { display: '指定顾客', name: 'CUST_NAME', width: "9%", sortable: false, align: 'left' },
                    { display: '使用门店', name: 'USED_STORE_NAME', width: "10%", sortable: false, align: 'left' },
                    { display: '使用日期', name: 'USED_TIME', width: "10%", sortable: false, align: 'left', process: formatDate },
        ...
        $('#btnDelete').click(function () {
            if (!confirm('确定要删除选中的代金券吗？')) return;
            var array = xjgrid.GetCheckedRowIds();
            if (array.length == 0) return;
            var ids = array.join(',');
            $.post(options.deleteBatchUrl, { ids: ids }, function (response) {
