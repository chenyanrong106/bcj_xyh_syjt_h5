; (function (window, undefined, $) {
    $(document).ready(function () {
        ...
        util.searchEmp = function (keyword) {
            $.getJSON(options.queryEmployeeByQ, { q: keyword }, function (res) {
                var _html = '';
                if (res != null && res.status > 0) {
                    var str = eval("(" + res.data + ")");
                    $(str).each(function () {
                        var li = '<li onclick=\"util.addEmp(this)\" id="li-' + this.id + '" itemid="' + this.value + '"> <div class="content"><div>' + this.id + '</div><div>' + this.value + '</div></li>'
                        _html = _html + li;
                    });
                } else {
                    _html = '<li>没有相关数据</li>';
                }
                $("#ulcontent").html(_html);
            });
        }
