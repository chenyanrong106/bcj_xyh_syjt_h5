; (function (window, undefined, $) {
    ...
    $(document).ready(function () {
        ...
        function InintView(bDate) {
            ...
            //检索员工姓名
            $.post(options.ENameList, { SCHEDULE_DATE: bStrDate,STORE_ID:storeId },
                    function (ret) {
                        if (ret.status > 0) {
        ...
        function getSchedule(obj,bDate,nums)
        {
            ...
            $.post(options.listUrl, { SCHEDULE_DATE: bStrDate, STORE_ID: storeId },
                     function (ret) {
                         if (ret.status > 0) {
                             var data = eval(ret.data);
                             var strHtml = [];
        ...
        $("#STORE_ID").change(function () {
            ...
            if (storeId == "")
            {
                _showInfoMessage('请您选择门店检索排班信息！', 'info');
                return false;
            }
            var date = new Date($("#scheduleDate").val());
            //重新加载刷新视图
                    ...
                    if (result.status == 0) {
                        ...
                        if (confirm(result.message)) {
                            ...
                            $.ajax({
                                url: options.deleteempUrl,
                                type: "POST",
                                data: { "MID": mid, "DATE": mdate, flag: $("#flag").val(), "SCHEDULESTARTDATE": $("#SCHEDULESTARTDATE").val(), "SCHEDULEENDDATE": $("#SCHEDULEENDDATE").val(), "SCHEDULE_TYPE": schedule_type },
                                success: function (result) {
...
function eventclick(obj) {
    ...
    $("input[name=SCHEDULE_TYPE]").each(function (i) {
       // alert(data[0] + "," + data[1] + "," + data[2] + "," + data[3] + "," + $(this).val());
        $(this).prop("checked", '');
        //$("#BEGINTIME").val("");
       // $("#ENDTIME").val("");
        if ($(this).val() == data[1]) {
