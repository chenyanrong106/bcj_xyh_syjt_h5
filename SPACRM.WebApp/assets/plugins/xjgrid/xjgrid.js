;(function(window,undefined,$){
	...
	function __Init__(options){
		...
		html.push("</div><div class='xjgrid-load'>加载中...</div>");
		html.push("</div>");
		//
		if(options.usepager){
			//分页的代码
	...
	function __BuildRowHtml__(data, options)
	{
	    ...
	    //alert(options.total); alert(options.data[0].cell[1]); alert(options.cols.length);
	    //for (var z = 0; z < options.data[0].ceil.length; z++) {
        //    alert(
	    //}
	    var html = [];
	    var k = options.colModel.length;
	    for (var i = 0, l = data.length; i < l && i < options.rp; i++) {
	        ...
	        for (var j = 0 ; j < k ; j++) {
	            ...
	            html.push("<td style='width:", col.width ? col.width : "auto", "'><div", style.join(""),  " name='"+col.name+i+"'>", text, "</div></td>");
	            //if(col.a)
	            //{
                //    alert(col.a);
	            //}
	    ...
	    if (options.sum) {
	        ...
	        for (var o = 0 ; o < k ; o++) {
	            ...
	            if (index == 0&&!col.sum) {
	                ...
	                html.push("<td style='text-align:center;'>小计</td>");
	            }
	            else if (col.sum) {
	                var sum = 0;
	                $("#" + options.elid + "_detail tbody tr").each(function () {
	                    //alert($(this).find("td").eq(o-1).find("div").html());
	                    sum += parseFloat($(this).find("td").eq(index).find("div").html());
	                });
	                var xsw = 2;
	                if (col.xsw || col.xsw == 0) {
	                    xsw = col.xsw;  //小数位，默认2位
	                }
	                html.push("<td><div style='text-align:left;'>" + sum.toFixed(xsw) + "</div></td>");
	            }
	            else {
	                html.push("<td>&nbsp;</td>");
	            }
	        ...
	        html.push("</tr>")
	        $("#" + options.elid + "_detail tbody").append(html.join(""));
	    }
	    //合计结束
	    ...
	    if (options.sum) {
	        ...
	        for (var p = 0 ; p < k ; p++) {
	            ...
	            if (index == 0 && !col.sum) {
	                ...
	                html.push("<td style='text-align:center;'>总计</td>");
	            }
	            else if (col.sum) {
	                var sum = 0;
	                for (var u = 0; u < options.data.length; u++) {
	                   // alert(options.data[u].cell[index] + "," + sum);
	                    sum += parseFloat(options.data[u].cell[p]);
	                };
	                var xsw = 2;
	                if (col.xsw||col.xsw==0) {
	                    xsw = col.xsw;  //小数位，默认2位
	...
	function __Query__(form, options) {     
	    ...
	    var p = { newp: 1, extparams: [], checkall: false, loaded: false };
	...
	function __QueryByFields__(search, options) {
	    var p = { newp: 1, extparams: search, checkall: false, loaded: false };
	    $.extend(options, p);
	    __LoadData__(options)
	    return false;
	}
