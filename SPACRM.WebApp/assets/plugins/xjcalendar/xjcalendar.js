; (function (window, undefined, $) {
    ...
    var __ITEMTTEMP__ = "<div style=\"width:${width};top:${top};left:${left};\" title=\"${tip}\" class=\"chip chip${i} ${drag}\"><div class=\"dhdV\" style=\"display:none\">${data}</div><div class='ct'>&nbsp;</div><dl style=\"border-left:${bdcolor} 2px solid;background-color:${bgcolor};height: ${height}px;\"><dt>${icon}${title}</dt><dd><div style=\"color:${color}\">${content}</div><div class='chip-tool'>${content2}</div></dd><div class='resizer' style='display:${redisplay}'><div class='rszr_icon'>&nbsp;</div></div></dl><div class='cb1'>&nbsp;</div><div class='cb2'>&nbsp;</div></div>";
    var __DRAGDATA__;
    var __DRAGEVENT__;
    var __STARTHOUR__ = 11;
    var __TIMESIZE__ = { "10": 15, "15": 17, "30": 25 }; // 支持的时间刻度
    var __TIMEHEIGHT__ = 17; // 每个单元格的高度
    var __TIMEUNIT__ = 15;  // 必须可以被60整除
    var __HOURHEIGHT__ = 17 * 4; // 一小时的高度
    var __TIMERCOUNT__ = 2; // 每一小时添加几个时间刻度；
    ...
    xjCalendar.prototype = {
        /**
         * 重置extWidth和__STARTHOUR__
         */
        __resetView__: function () {
            var width = this.options.itemWidth * (this.options.colModels.length);  //总长度,其中17为滚动条宽度
            if (width + 20 < this.options.width) {
                this.options.extWidth = this.options.width - width;
            }
            //开始时间 小时
            __STARTHOUR__ = this.options.rowRange.start;
            __TIMEUNIT__ = this.options.timeUnit;
            __TIMEHEIGHT__ = __TIMESIZE__[__TIMEUNIT__];
            if (!__TIMEHEIGHT__) {
                alert("该时间刻度不被支持");
                throw "该时间刻度不被支持";
            }
            __TIMERCOUNT__ = 30 / __TIMEUNIT__;
            __HOURHEIGHT__ = __TIMERCOUNT__ * __TIMEHEIGHT__ * 2;
        },
        /**
         * 初始化
         */
        ...
        render: function () {
            ...
            if (this.options.enablefullscreen) {
                html.push("<tr><td><div class=\"fix-tb-top\"><a href='javascript:;' id='xjcal_btn_fullscreen' class='xjcal-btn-fullscreen'><i class='fa fa-arrows-alt'></i></a></div></td></tr>");
            }
            else {
            ...
            for (var j = 0, l = this.options.colModels.length; j < l; j++) {
                html.push("<td class='itemWidth' style='width:", this.options.itemWidth, "px'><div class=\"data-main-top itemWidth-1\"  style='width:", this.options.itemWidth - 1, "px'>", this.options.colModels[j].display, "</div></td>");
            };
            html.push("<td><div class=\"data-main-top extWidth-1\" style='width:", this.options.extWidth - 1, "px'>&nbsp;</div></td>");
            html.push("</tr>");
        ...
        */
        bind_event: function () {
            ...
            $("#xjcal_btn_fullscreen").click(function () {
                self.toggleFullScreen();
            });
        },
        /**
         * 扩展参数
         * @param  {Object} p [需要参数的内容]
         */
        ...
        */
        toggleFullScreen: function () {
            ...
            if (this.options.fullscreen) {
                ...
                this.el.removeClass("xjcalendar-full");
                //重新设置宽高
                //先把原来的保存起来
                ...
                this.el.addClass("xjcalendar-full");
        ...
        */
        resize: function (w, h) {
            if (w) {
                this.setoption({ width: w });
            }
            if (h) {
                this.setoption({ height: h });
            }
            this.__resetView__();
            //设置容器的样式宽度高度
            this.el.css({ width: (this.options.width + 60) + "px", height: (this.options.height + 30) + "px" });//+492
            this.render();
        },
        /**
         * 动态修改列宽
        */
        resizeItemWidth: function () {
            ...
            var extWidth = this.options.extWidth;
            // 修改所有itemWidth 参数影响的
            this.el.find("td.itemWidth").each(function () {
                $(this).css({ width: itemWidth });
            });
            this.el.find("div.itemWidth-1").each(function () {
                $(this).css({ width: itemWidth - 1 });
            });
            this.el.find("div.extWidth-1").each(function () {
                $(this).css({ width: extWidth - 1 });
            });
            this.resizing = false;

        },
        resizeTimeHeight: function (unit) {
            if (__TIMEUNIT__ == unit) {
                return false;
            }
            this.setoption({ 'timeUnit': unit });
            this.__resetView__();
            this.render();
        }
    ...
    function _GetPHM_(ts1, ts2) {
        var t1 = ts1 / __HOURHEIGHT__;
        var t2 = parseInt(t1);
        var t3 = __GetQuarterTime__(t1, t2);
        var t4 = ts2 / __HOURHEIGHT__;
        var t5 = parseInt(t4);
    ...
    function __GetHM__(y1, y2, pt) {
        ...
        var t1 = (sy1 - pt) / __HOURHEIGHT__;  //计算当前的小时
        var t2 = parseInt(t1);
        var t3 = __GetQuarterTime__(t1, t2);
        var t4 = (sy2 - pt) / __HOURHEIGHT__; //计算当前的小时
        var t5 = parseInt(t4);
        var t6 = __GetQuarterTime__(t4, t5);
    ...
    function __GetPosition__(h, m) {
        //console.log("starthour"+__STARTHOUR__)
        h = h - __STARTHOUR__;
        return h * __HOURHEIGHT__ + parseInt(m / 60 * __HOURHEIGHT__) - 2;
    }
    ...
    function __RenderMainTable__(start, end, colModels, items, html, options) {
        ...
        for (var j = 0, l = colModels.length; j < l; j++) {
            html.push("<td class='itemWidth' style='width:", options.itemWidth, "px'><div class=\"data-main-top itemWidth-1\" style='width:", options.itemWidth - 1, "px'></div></td>");
        };
        html.push("</tr>");
        html.push("<tr>");

        for (var j = 0, l = colModels.length; j < l; j++) {
            html.push("<td style=\"border:none;padding-right:1px;\">");
            ...
            if (colModels[j].shm != "" && colModels[j].ehm != "") {
                ...
                html.push("<div class=\"tgItemBG itemWidth-1\" style=\"top:", sP + 3, "px;height:", h, "px;width:", options.itemWidth - 1, "px\"></div>");
            }

            html.push("<div id=\"tgspanningwrapper\" class=\"tg-spanningwrapper\"><div class=\"tg-hourmarkers\">");
            for (var i = 0; i < (end - start + 1) * __TIMERCOUNT__; i++) {
                html.push("<div class=\"tg-dualmarker\" style='font-size:", __TIMEHEIGHT__, "px'></div>");
        ...
        //console.log("p1:" + mHg);
        var dataheight = (end - start + 1) * __HOURHEIGHT__ - 2;
        html.push("<tr>");
        for (var j = 0, l = colModels.length; j < l; j++) {
    ...
    function __BuildItem__(theme, item, index) {
        ...
        var p = { bdcolor: theme[0], bgcolor: theme[1], color: theme[2] };

        var content = [];

        content.push("<div><a href='/Customer/View360.do' style='color:", p.color, ";'>", item.item.UserFullName, "</a></div>");

        content.push("<div>", item.item.MobilePhone, "</div>");
        ...
        p.title = "<a href='javascript:void(0);' title='点击切换的客户360视图' class='chip-user'>&nbsp;</a>"; // "人名";//" + item.item.UserFullName + "
        ...
        if (item.item.Status == 0) {
            //新预约
            icons.push("<i class='cal-icon icon-new'  style='margin:0 5px 0 5px;color:", p.color, "'></i>");
            //icons.push("<i title='新预约' class='fa fa-github-alt' style='margin:0 5px 0 5px;color:",p.color,";'></i>");
        }

        if (item.item.UserCard && item.item.UserCard != "0") {
            //是否会员 卡号
            icons.push("<i class='cal-icon icon-member'  style='margin:0 5px 0 5px;color:", p.color, "'></i>");
        }
        p.icon = icons.join("");
        ...
        if (item.item.Is_Confirm == "1")
            tools.push("<a title='电话已确认' abbr='3' href='#'><i class='fa fa-phone'></i><span style='margin-left:2px'></span></a>");
        if (item.item.Remark != "" && item.item.Remark != null) {
            tools.push("<a title='备忘录' abbr='3' href='#'><i class='fa fa-bookmark'></i><span style='margin-left:2px'></span></a>");
        }
        if (item.item.Status == "2") {
            tools.push("<a title='已付款' abbr='3' href='#'><i class='fa fa-cny'></i><span style='margin-left:2px'></span></a>");
        }
        if (item.item.IsStaff == true) {
            tools.push("<a title='指定技师' abbr='3' href='#'><i class='cal-icon icon-staff'></i><span style='margin-left:2px'></span></a>");
        }
        console.log(item.item.Source);
        if (item.item.Source == "0") {
            tools.push("<a title='电话预约' abbr='3' href='#'><i class='fa fa-mobile'></i><span style='margin-left:2px'></span></a>");
        } else if (item.item.Source == "2") {
            tools.push("<a title='微信预约' abbr='3' href='#'><i class='fa fa-weixin'></i><span style='margin-left:2px'></span></a>");
        } else if (item.item.Source == "3") {
            tools.push("<a title='直接到店' abbr='3' href='#'><i class='fa fa-home'></i><span style='margin-left:2px'></span></a>");
        }
        p.content2 = tools.join("");
    ...
    function __BuildTempItem__(data, h) {
        var theme = __ThemeControl__(1); //TODO； 如果获取默认主题？
    ...
    function __ThemeControl__(d) {
        function zc(c, i) {
            ...
            var d = "d075dbf6e2f78d308e57d957def7da00892dff2667ffd4dfd52a5e00aff2cfeffc1c77aaff9334fce6ccd7652cffca45fff4d0d5934da38363ede6df9f5f40";
            //var color = "#" + d.substring(c * 30 + i * 6, c * 30 + (i + 1) * 6);
            var color = "#" + d.substring(c * 18 + i * 6, c * 18 + (i + 1) * 6);
            return color;
        }

        //return [zc(d, 0), zc(d, 1), zc(d, 2), zc(d, 3)];
        return [zc(d, 0), zc(d, 1), zc(d, 2)];
    }
    ...
    function __RenderHoursTb__(start, end, html) {
        ...
        for (var i = start; i <= end + 1; i++) {
            html.push("<div style=\"height: ", __HOURHEIGHT__, "px;line-height:", __HOURHEIGHT__, "px;", i == end ? "border-color:#fff;" : "", "\" class=\"tg-time\"><span>", i, "</span><em>30</em></div>");
        }
        html.push("</td></tr>");
    }
