; (function (window, undefined, $) {
    $(document).ready(function () {
        ...
        function custFormatResult(obj) {
            ...
            if (obj.cust_card_id !== undefined && obj.cust_card_id != 0) {
                if (obj.card_name != '') {
                    markup += "<div class='movie-synopsis'>" + obj.card_name + "(" + obj.balance + ")" + "</div>";
                }
            }
            markup += "</td></tr></table>"
            return markup;
        ...
        function getCustomerInfo2(cid, custInfo) {
            $("#spanCustName").html("");
            $("#spanCustMobile").html("");
            $("#spanCustCard").html("");
            ...
            if (custInfo) {
                $("#hide_Cid").val(cid);
                $("#spanCustName").html(custInfo.title);
                $("#spanCustMobile").html(custInfo.mobile);
                $("#spanCustCard").html(custInfo.card_no);
                $("#spanCustCardLevel").html(custInfo.card_name);
                $("#spanCustCardBalance").html(custInfo.balance);
                $("#divNewCustomer").hide();
                $("#divCustomer").show();
        ...
        function getCustomerInfo(cid, cname) {
            ...
            $("#spanCustCardBalance").html("");
            if (cname == "散客")
            { $("#buycard").hide(); $("#filelist").hide(); $("#WALKIN_NUM").val(1); $("#MEMBER_NUM").val(0); }
            else {
            ...
            $.post(options.customerDetailUrl, { id: cid, name: cname },
                  function (data) {
                      if (data.status == 1) {
                          if (cid <= 0) {
                              $("#hide_Cid").val(data.data.ID); console.log(data.data.ID);
                          }
                          else {
                              $("#hide_Cid").val(cid);
                          }
                  ...
                  function (data) {//oidx订单id
                      if (data.status > 0) {
                          ...
                          for (var i = 0; i < strJson.length; i++) {
                              ...
                              //var PROD_AMT = strJson[i].ADJUST_AMT;
                              var IS_CARDPAY = strJson[i].IS_CARDPAY;
                              var IS_DISCOUNT = strJson[i].IS_DISCOUNT;
                              ...
                              html.push('<input id="' + empdivid + '" onclick="openwin(this.id)" readonly="readonly" value="' + EMP_NAME + '"  style="width:80px;"/>');
                              html.push('&nbsp;<input type="checkbox" name="cbxIS_DK" "' + checked + '" onchange="cbxIS_DKChange(this)" />点钟&nbsp;');
                              html.push('</td>');
                              ...
                              html.push('<input type="hidden" name="ADJUST_AMT" value="' + strJson[i].ADJUST_AMT + '" />');//调整金额
                              html.push('<input type="hidden" name="ADJUST_REASON" value="' + strJson[i].ADJUST_REASON + '" />');//调整原因
                              //点钟
        ...
        function getPROMOTION(PROD_ID, cidx, PROD_TYPE, proi, PROMOTION_ID, PROD_PRICE) {
            ...
            function (res) {
                if (res.status > 0) {
                    ...
                    $("#selectPrivilege_" + PROD_ID + "_" + cidx + "_" + PROD_TYPE + "_" + proi).append("<option value='-1'>直接改价</option>");
    ...
    function getProAjax(pid, table, name, obj, cust_cdetail_id) {
        ...
        var lastv = "0";
        try {
            lastid = ($('#tableProds').find('tbody tr').last().find("td").eq(2).find("input").eq(0).val());
            lastv = $('#tableProds').find('tbody tr').last().find("td").eq(6).find("input[name=Employee]").val();
        } catch (e) {
        ...
        html.push('<input id="' + empdivid + '" onclick="openwin(this.id)" readonly="readonly" value="' + lastid + '" style="width:80px;">');
        html.push('&nbsp;<input type="checkbox" name="cbxIS_DK" onchange="cbxIS_DKChange(this)" />点钟&nbsp;');
        html.push('</td>');
        if (cust_cdetail_id != undefined
            && cust_cdetail_id != null
            ...
            html.push('<input type="hidden" name="PRODGETTYPE" value="' + objcell[1] + '" />');//疗程类型 1购买的疗程 2赠送的疗程 
            html.push('<input type="hidden" name="Employee" value="' + lastv + '" /> ');
            html.push('<input type="hidden" name="ADJUST_AMT" value="0" />');//调整金额
            html.push('<input type="hidden" name="ADJUST_REASON" value="" />');//调整原因
            //点钟
            html.push('<input type="hidden" name="IS_DK" value="false" />');
        ...
        $.ajax({
            ...
            success: function (rets) {
                ...
                $("#selectPrivilege_" + pid + "_" + cid + "_" + table + "_" + proi).append("<option value='-1'>直接改价</option>");
                proi++;
    ...
    $("#btnCheckIn").click(function () {
        var MEMBER_NUM = $("#MEMBER_NUM").val();//会员人数
        var WALKIN_NUM = $("#WALKIN_NUM").val();//散客人数
        var ServiceNum = $("#tableProds").find("tbody").find("tr").length; //订购项目数
        if (parseInt(MEMBER_NUM) +parseInt(WALKIN_NUM) !=parseInt(ServiceNum)) {
            if (confirm("会员人数与散客人数之和不等于订购项目数，是否继续？")) {
...
function privilege(obj) {
    ...
    if (id > 0) {
        ...
        //bootbox2.prompt({
        //    title: "店长身份验证", callback: function (result) {
        //        if (result === null) {
        ...
        //                dataType: "json",
        //                url: "GetManagerPass.do?pass=" + result,   //提交的页面/方法名
        //                data: "{'pass':'" + result + "'}",              //参数（如果没有参数：null）
        ...
        //                    if (data.status > 0) {
        bootbox.prompt("直接改价", function (res) {
            if (res === null) {
                ...
                ////剩下现金支付
                //$("#spanCashPay").html(cashPayprice);
                $("#spanCashPay").html($("#spanZJ").html());  //现金
                $("#spanCardPay").html(($("#spanZJ").html() * 0.5).toFixed(0));  //五折卡
                $("#spanCardDistPay").html(($("#spanZJ").html() * 0.6).toFixed(0));  //六折卡
                //_showInfoMessage('项目[' + name + ']添加成功！', 'success');
                if ($("#iska").val() == 1) {
                    var je = parseInt($("#spanZJ").html()) - ($("#spanCustCardBalance").html() / $("#DISCOUNT_RATE").val()).toFixed(0);
                    if (($("#spanCustCardBalance").html() / $("#DISCOUNT_RATE").val()).toFixed(0) >= parseInt($("#spanZJ").html())) {
                        je = 0;
                    }
                    $("#hyktx").html("当前" + $("#spanCustCardLevel").html() + "卡中剩余金额" + $("#spanCustCardBalance").html() + "元，不充值还需支付现金:" + je);
                }
                //end
                ...
                bootbox.prompt("改价原因", function (reason) {
                    if (reason === null) {

                    } else {
                        $(obj).parent("td").parent("tr").find("input[NAME$=ADJUST_AMT]").val(res * -1);
                        $(obj).parent("td").parent("tr").find("input[NAME$=ADJUST_REASON]").val(reason);
                    }
                });
...
function addEmp(obj) {
    ...
    $(id).val($(obj).attr("id"));//当前激活员工文本框
    $(id).parent("td").parent("tr").find("input[NAME$=Employee]").val($(obj).attr("itemid"));
    $("#closewin").click();
}
