    ...
    $(document).ready(function () {
        ...
        var gridopt = {
            ...
            colModel: [
                    ...
                    { display: '姓名', width: "8%", name: 'CUST_NAME', sortable: false, align: 'left' },
                    { display: '手机号', name: 'CUST_MOBILE', sortable: false, align: 'left' },
                    ...
                    { display: '店名', name: 'STORE_NAME', sortable: false, align: 'left' },
                    ...
                    { display: '是否微信确认', name: 'IS_WX_CONFIRM', hide: true, sortable: false, align: 'left' },
                    //{ display: '预约提交时间', name: 'CREATE_DATE', sortable: false, align: 'left' },
                    { display: '微信确认时间', name: 'WX_CONFIRM_DATE', sortable: false, align: 'left' },
                    { display: '微信确认人', name: 'WX_CONFIRM_STAFFNAME', sortable: false, align: 'left' },              //10
                    { display: '操作', name: 'ID', sortable: false, align: 'center', process: processOp },                //11
        ...
        function processOp(value, cell) {
            ...
            if (status == "TRUE") {
                ops.push("&nbsp;<span title='已微信确认' class='abtn' >已确认</span>");
            }
            else {
                ops.push("&nbsp;<a title='微信确认' class='abtn' href='javascript:;'  onclick=\"util.Confirm('", value, "','", cell[1], "')\"><i class='fa fa-edit' ></i>微信确认</a>");
        ...
        $("#btnConfirmOk").click(function (e) {
            $('#confirmModal').modal('hide');
            var id = $("#hdCurrentId").val();
            $.post(options.setStatusUrl + "/" + id, { id: id, status: 0 },
                  function (res) {
                      if (res.status > 0) {
                          //showLoadingMsg("操作成功", { right: 100, top: 10 }, true, 5000);
                          if (res.message && res.message != "") {
                              _showInfoMessage("操作成功，微信发送失败，" + res.message, 'error');
                          }
                          else {
                          ...
                          //showErrorTip("操作失败！：" + res.message, { right: 100, top: 10 }, true, 5000);
                          _showInfoMessage("页面有异常：" + res.message, 'error');
                          xjgrid.Reload();
                      }
                  },
                  "json"
